...
class buymodel{
    ...
    public function show($conn,$userid){ //Shows all the products for specific user
        ...
        while($row = mysqli_fetch_array($result)){            
            ...
            $time = $row['Time'];
            $status = " ";
            $statusinsert =  "INSERT INTO `Status`(`Userid`, `ProductId`, `status`,`purchased_on` ) VALUES ('$Uid','$Pid','$status','$time' )";    
    ...
    public function vieworders($conn,$date ,$userid){ //Joining the table to view orders
            $pattern = "/[-\s:]/";
            ...
            $query = "SELECT   *
              ...
              WHERE orderhistory.UserId = '$userid' AND orderhistory.Time LIKE '$year-$month-$datee %:%:%' AND Status.Userid='$userid' ";

             //$query = "SELECT * FROM `orderhistory` WHERE `UserId`= '$userid' AND `Time` LIKE '$year-$month-$datee %:%:%' ";
